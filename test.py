...
if __name__ == '__main__':
    ...
    my = json.dumps(login_data)
    print(type(my))
    result = re.findall(r'#(.*?)#', my)
    for i in result:
        my = my.replace(f'#{i}#', f'{user_list[0]}')
    my = json.loads(my)
    print(my)

    # asyncio.run(main(data_list=user_list, method=request_method, base_url=base, login_url=url, login_data=login_data,
    #                  my_header=None))
    # with open('token.txt','a+') as f:
    #     f.write('11111')
